...
export default function Landing() {
  ...
  const [generatedImageUrl, setGeneratedImageUrl] = useState(); // openai返回的是url
  const [slicedPhotos, setSlicedPhotos] = useState(); // 展示给用户的图片
  ...
  const createImage = async () => {
    try {
      ...
      const response = await openai.images.generate({ // 调用openai API
        model: "dall-e-3",
        prompt: inputValue,
        n: 1, // 生成图片数量，要花钱！！！！！！！！！！！！
        size: "1024x1024",
        ...
        toolbar={{
          buttons: [
            <IconButton
              ...
              aria-label="search"
              onClick={async () => {
                fetch(generatedImageUrl.replace(/https:\/\/[^\/]+/, "/api")) // 修改图片地址 把很多东西替换成 /api
